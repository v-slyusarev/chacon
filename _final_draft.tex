...
\item перед $*$ находится элемент, отличный от $0$.
\end{itemize}
\end{definition}
...
\begin{itemize}
\item Пусть $X$ --- регулярное множество, не содержащее $[0]$. Тогда $SX$ --- также регулярное множество, не содержащее $[0]$
\item Пусть $X$ --- регулярное множество, содержащее $[0]$. Тогда
$$SX :=  \{Sx \big| x \in X\} = A \cup Y,$$
где $A$ и $Y$ --- регулярные множества, причём $A$ не содержит $[0]$.
\end{itemize}
\end{proposition}
\begin{proof}
Первый случай разобран в \cref{SA}, докажем второй.
...
Первое множество не содержит $[0]$, действие оператора $S$ на него может быть вычислено по правилу из \cref{SA}. Очевидно, что оно регулярно и не содержит $[0]$. Положим $A = S(x_0 x_1 \ldots x_{k-1} x_{k+1} \ldots x_n*)$

Рассмотрим второе множество $X' := (x_0 x_1 \ldots x_{k-1}  0 [0] x_{k+1} \ldots x_n*)$. Если $x_0 = x_1 = \ldots = x_{k-1} = (p-1)$, то легко убедиться в том, что $SX' = 0 0 \ldots 1 [0] x_{k+1} \ldots x_n *$. Иначе для любого $x \in X'$ оператор $S$ действует только на первые $k-1$ координат $x$, и образ остаётся в $X'$. В любом случае получается регулярное множество $Y := S X'$, содержащее $[0]$.
\end{proof}
...
\begin{itemize}
\item Если для некоторого $k \in [0, n]$ $x_k = [0]$, то $\lambda(X)=\frac{1}{p^n(p-1)}$.
...
\sum\limits_{l = 0}^{\infty} \frac{1}{p^{n+l+1}} = \frac{1}{p^{n+1}}\sum\limits_{l = 0}^{\infty} \frac{1}{p^l}  = \frac{1}{p^{n+1}}\frac{p}{p-1} = \frac{1}{p^n(p-1)}$$
...
Повторяем описанную процедуру для $m \in [1, M-1]$. 

\begin{figure}[!h]
\Tree[.(start) [.$X_1$  [.$A_1$ [.$B_1$ [.$\ldots$ ] ] ]
                        [.$Y_1$ [.$C_1$ [.$\ldots$ ] ]
                                [.$Z_1$ [.$\ldots$ ] ] ] ]
               [.$X_2$  [.$A_2$ [.$B_2$ [.$\ldots$ ] ] ]
                        [.$Y_2$ [.$C_2$ [.$\ldots$ ] ]
                                [.$Z_2$ [.$\ldots$ ] ] ] ]
               [.$\ldots$ ]
               [.$X_{p-1}$ [.$A_{p-1}$ [.$B_{p-1}$ [.$\ldots$ ] ] ]
                           [.$Y_{p-1}$ [.$C_{p-1}$ [.$\ldots$ ] ]
                                       [.$Z_{p-1}$ [.$\ldots$ ] ] ] ] ]
    \caption{\footnotesize{Схема первой фазы алгоритма}}
\label{fig:algorithm_first_phase}
\end{figure}
На \cref{fig:algorithm_first_phase} показаны первые шаги алгоритма. Ребро дерева соответствует действию оператора $S$. Буквами $X, Y, Z$ помечены регулярные множества, содержащие $[0]$; буквами $A, B, C$ --- регулярные множества, не содержащие $[0]$.
...
По завершении этой процедуры в нижнем уровне дерева будут выписаны пары $\left(X, f(X)\right)$, где $X$ --- регулярное множество, $f(X)$ --- вычисленное для данного узла значение $\Phi^{(m)}$. Пользуясь \cref{regular_proba}, можно вычислить вероятность $\lambda(X)$. Отметим, что у всех выписанных пар не обязательно различаются значения функционала, так что полученные вероятности следует просуммировать:
 $$\pi_M(j) = \lambda\{\Phi^{(m)}(x) = j\} = \sum\limits_{f(X)=j} \lambda(X)$$
 
 По построению $f(X) = \Phi^{(M)}(S^{-(M-1)}X)$, так что по выписанным в листах дерева регулярным множествам $X$ можно восстановить аргументы функционала. Задача решена.
